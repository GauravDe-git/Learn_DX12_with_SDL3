#include "../include/CommandAllocatorPool.hpp"
...
ID3D12CommandAllocator* CommandAllocatorPool::RequestAllocator(uint64_t CompletedFenceValue)
{
    ...
    {
        std::pair<uint64_t, ID3D12CommandAllocator*>& AllocatorEntry = m_ReadyAllocators.front();

        if (AllocatorEntry.first <= CompletedFenceValue)
        {
            pAllocator = AllocatorEntry.second;
            pAllocator->Reset();
    ...
    if (pAllocator == nullptr)
    {
        ...
        m_Device->CreateCommandAllocator(m_cCommandListType, IID_PPV_ARGS(&pAllocator));
